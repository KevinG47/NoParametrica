...
resultado_mannwhitney <- wilcox.test(TOTAL_GN ~ TORNEO, 
                                     data = datos_goles,
                                     paired = FALSE,  # Para Mann-Whitney (independientes)
                                     exact = FALSE) # aproxima el p_valor por tantos empates 
...
ggplot(datos_goles, aes(x = TORNEO, y = TOTAL_GN, fill = TORNEO)) +
  ...
  theme_minimal()
#se ve una diferencia en el total de goles debido a q en el mundial el 26% de sus datos son prorroga. penalties en cambio en eurocopa el 37% de sus datos son prorroga o penalties haciendo q hayan mas partidos eque acabaron en el tiempo reglamentarion en el mundial habiendo mayor cantidad de goles totales en los 90 minutos
#tabla 1################################################################
...
view(tabla_final_fisher%>%select(Comparacion,Prop_Año1_Porc,Total_Año1,Prop_Año2_Porc,Total_Año2,Odds_Ratio, P_Valor))
